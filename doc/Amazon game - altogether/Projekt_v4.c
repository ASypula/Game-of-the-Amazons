...
void place_amazons(int number_of_players, player_data *player, tile **board, int number_of_pawns){
...
{
    printf ("%s, please choose a tile with your amazon: ", player[current_player].name);
    get_move (x, y, c, m, n);
...
int main()
{
    ...
    while (k < m)
    {
        ...
        while (j < n)
        {
            board[k][j].treasure = 0;
            board[k][j].artifact = 0;
            board[k][j].occupation = 0;
            j++;
        }
        k++;
    }
    ...
    place_amazons(number_of_players, player, board, number_of_pawns);
